...
const snakepublicCtgData = require("../models/snakepublic");
...
router.post("/snakepublicctg", async (req, res) => {
  try {
    // Validate the incoming data to ensure it follows the schema
    const newRow = new  snakepublicCtgData(req.body);
...
router.delete("/snakepublicctg/:id", async (req, res) => {
  try {
    const _id = req.params.id.trim(); // Trim the ID to remove any whitespace or newline characters
    ...
    const deletedData = await snakepublicCtgData.findByIdAndDelete(_id);
