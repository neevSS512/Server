...
router.patch("/ludoscorectg/:id", async (req, res) => {
  try {
    ...
    const updatedData = await ludoscoreCtgData.findByIdAndUpdate(_id, req.body, { new: true });
...
router.post("/ludoscorectg", async (req, res) => {
  try {
    // Validate the incoming data to ensure it follows the schema
    const newRow = new ludoscoreCtgData(req.body);
